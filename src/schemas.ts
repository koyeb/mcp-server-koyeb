...
function buildpackBuilder() {
  return z.object({
    build_command: z
      .string()
      .optional()
      .describe('A command used to override the build command, run after all build steps'),
...
function git() {
  return z.object({
    branch: z
      .string()
      .optional()
      .describe('A git branch that will be tracked for new commits and deployments will be created'),
    build_command: z
      .string()
      .optional()
      .describe(
    ...
    no_deploy_on_push: z
      .boolean()
      .optional()
      .describe('A flag to disable a new deployment when a push event is detected'),
    repository: z
      .string()
      .optional()
      .describe(
...
function httpHealthCheck() {
  return z.object({
    headers: httpHeader()
      .array()
      .optional()
      .describe('An optional list of HTTP headers to provide when performing the request, default is empty'),
    method: z
      .string()
      .optional()
      .describe('An optional HTTP method to use to perform the health check, default is GET'),
    path: z.string().optional().describe('The path to use to perform the HTTP health check'),
    port: z
      .number()
      .optional()
      .describe(
        'The port to use to perform the health check, must be declared in the ports section. Format: int64',
      ),
  });
}
...
function healthCheck() {
  return z.object({
    grace_period: z
      ...
      .optional()
      .describe(
        'An optional initial period in seconds to wait for the instance to become healthy, default is 5s. Format: int64',
      ),
    http: httpHealthCheck().optional(),
    interval: z
      .number()
      .optional()
      .describe('An optional period in seconds between two health checks, default is 60s. Format: int64'),
    restart_limit: z
      .number()
      .optional()
      .describe(
        'An optional number of consecutive failures before attempting to restart the service, default is 3. Format: int64',
      ),
    ...
    timeout: z
      ...
      .optional()
      .describe(
        'An optional maximum time to wait in seconds before considering the check as a failure, default is 5s. Format: int64',
      ),
  });
}
  ...
  return z.object({
    ...
    type: z.union(
      instances.map((instance) =>
        z.literal(instance.type).describe(getInstanceDescription(instance)),
      ) as unknown as [z.ZodString, z.ZodString, ...z.ZodString[]],
    ),
  });
}
...
function scalingTarget() {
  return z.object({
    value: z.number().describe('Format: int64'),
  });
}

function deploymentScalingTarget() {
  return z.object({
...
function volume() {
  return z.object({
    ...
    replica_index: z
      .number()
      .optional()
      .describe('optionally, explicitly choose the replica index to mount the volume to. Format: int64'),
    scopes: z.string().array().optional().describe('scope of the associated'),
  });
}

export function deploymentDefinitionSchema() {
  return z.object({
